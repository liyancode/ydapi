  ...
  module BizModel
    module DBModel
      class Customers < Sequel::Model(:customers)
        ...
        def Customers.func_add(customer)
          begin
            Customers.create do |c|
        ...
        def Customers.func_delete(customer_id)
          begin
            Customers[customer_id: customer_id].update(status: 0)
            true
          rescue Exception => e
        ...
        def Customers.func_update(customer)
          begin
            exist_customer=Customers[customer_id: customer.customer_id]
        ...
        def Customers.func_get(customer_id)
          begin
        ...
        def Customers.func_get_by_added_by_user_name(added_by_user_name)
          begin
            Customers.dataset.where(added_by_user_name:added_by_user_name).where(status:1).all
          rescue Exception => e
        ...
        def Customers.func_get_max_customer_id
          begin
            Customers.last.customer_id
          rescue Exception => e
